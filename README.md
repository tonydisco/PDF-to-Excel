...
Chọn được nhiều file PDF cùng lúc (tối đa **10 file**/lần). Mỗi PDF cho ra một file Excel cùng tên (`Tên_file.xlsx`).
...
| **macOS** | Mở Terminal, chạy: `brew install tesseract tesseract-lang` |

> Gói tiếng Việt (`vie.traineddata`) đã nằm sẵn trong thư mục `tessdata/` của ứng dụng, nên trên Windows bạn **không cần** tự tải.
...
### Cách 2 — Bản đóng gói (.exe/.app) — xem mục 5

### Thao tác trong cửa sổ

1. Bấm **➕ Thêm file** → chọn 1–10 file PDF.
2. Chọn **Thư mục lưu Excel** (mặc định tạo thư mục `Excel_output` cạnh file đầu tiên).
3. Bấm **🚀 CHUYỂN ĐỔI**. Theo dõi thanh tiến trình và **Nhật ký**.
4. Xong, ứng dụng tự mở thư mục chứa kết quả.
...
Nếu **khớp**, Nhật ký báo **✓ OK**. Nếu **lệch**, báo **⚠**. Vì số liệu sai do OCR gần như luôn làm vỡ cân đối, đây là tín hiệu chất lượng rất đáng tin.

> Với báo cáo quan trọng: ưu tiên kiểm tra các **ô tô cam** và bật **"Chất lượng cao"** (đọc 3 độ phân giải).

---

## 5. Đóng gói thành .exe / .app (để chia sẻ cho máy khác)
...
│   └── engine.py          # điều phối + kiểm tra cân đối
